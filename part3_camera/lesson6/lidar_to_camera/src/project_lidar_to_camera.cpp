...
void projectLidarToCamera2()
{
    ...
    cv::Mat P_rect_00(3,4,cv::DataType<double>::type); // 校正后的3x4投影矩阵
    cv::Mat R_rect_00(4,4,cv::DataType<double>::type); // 3x3校正旋转以使图像平面共面
    cv::Mat RT(4,4,cv::DataType<double>::type); // 旋转矩阵和转换向量
